...
완벽합니다! 모든 컴포넌트와 url을 전달받는 고차 컴포넌트를 만들었습니다.
...
2. 데이터를 가져온 후에는 가져온 데이터와 동일한 `data`를 설정합니다.
   ...
   그렇다면 어플리케이션에 이 동작을 추가해서 `DogImages` 목록에 Loading...을 표시하려면 어떻게 해야 할까요?

우리는 `DogImages.js`에서 더 이상 DogImages 컴포넌트 그 자체를 반환하는 걸 원하지 않습니다.
대신, DogImages 컴포넌트를 `withLoading`으로 "래핑하는" 고차 컴포넌트를 반환하고자 합니다.
...
완벽합니다!
우리는 이제 데이터가 불러와질 때 Loading... 이 화면에 뜨는 것을 볼 수 있습니다.

고차 컴포넌트 패턴은 모든 로직을 한 곳에 두면서, 여러 개의 컴포넌트에 동일한 로직을 적용하도록 합니다.
withLoader 고차 컴포넌트는 전달받는 컴포넌트나 url를 신경쓰지 않습니다:그것들이 유효한 컴포넌트,API 엔드포인트인 이상,
그저 API 엔드포인트에서 가져온 데이터를 우리가 전달한 컴포넌트에 전달합니다.
...
전달할 요소에 `hovering` 프로퍼티를 제공하는 고차 컴포넌트를 만들어야 합니다. <br/>
이 프로퍼티를 기반으로 사용자가 `DogImages` 목록 위로 마우스를 가져가면 텍스트 상자가 렌더링됩니다.
...
> 고차 컴포넌트를 작성하는 데 사용되는 잘 알려진 라이브러리는 [recompose](https://github.com/acdlite/recompose)입니다. 고차 컴포넌트는 대부분 리액트 훅으로 대체할 수 있기 때문에 recompose 라이브러리는 더 이상 유지되지 않으므로 이 글에서는 다루지 않겠습니다.

## Hooks

경우에 따라 고차 컴포넌트 패턴을 리액트 훅으로 대체할 수 있습니다.

`withHover` 고차 컴포넌트를 `useHover` 훅으로 대체해 보겠습니다. <br/>
고차 컴포넌트 대신 mouseOver와 mouseLeave 이벤트 리스너를 요소에 추가하는 훅을 내보냅니다.  <br/>
더이상 고차 컴포넌트에서 했던 것처럼 요소를 전달할 수 없습니다. 대신 mouseOver,mouseLeave 이벤트를 가져와야 하는 훅에서 ref를 반환합니다.
...
`useEffect` 훅은 컴포넌트에 이벤트 리스너를 추가하고, 사용자가 요소 위에 마우스를 올려놓았는지 여부에 따라 `hovering` 값을 true/false로 설정합니다. ref와 `hovering` 값 모두 훅에서 반환해야 합니다:mouseOver,mouseLeave 이벤트를 받는 컴포넌트에 참조를 추가하기 위한 `ref`, 조건부로 Hovering! 텍스트 상자를 렌더링하기 위한 `hovering`

`DogImages` 컴포넌트를 withHover 고차 컴포넌트로 감싸는 대신, `DogImages` 컴포넌트 내부에서 바로 useHover 훅을 사용할 수 있습니다.
...
일반적으로 React 훅은 고차 컴포넌트 패턴을 대체하지 않습니다.

_"대부분의 경우 훅으로 충분하며 트리에서 중첩을 줄이는 데 도움이 될 수 있습니다." - [React 공식문서](https://legacy.reactjs.org/docs/hooks-faq.html#do-hooks-replace-render-props-and-higher-order-components)_

React 문서에 따르면 훅을 사용하면 컴포넌트 트리의 깊이를 줄일 수 있습니다.
고차 컴포넌트 패턴을 사용하면 컴포넌트 트리가 깊게 중첩되기 쉽습니다.
...
컴포넌트에 직접 훅을 추가하면 더 이상 컴포넌트를 래핑할 필요가 없습니다.

고차 컴포넌트를 사용하면 동일한 로직을 한 곳에 유지하면서 여러 컴포넌트에 동일한 로직을 제공할 수 있습니다. 훅을 사용하면 컴포넌트 내에서 사용자 정의 동작을 추가하므로, 여러 컴포넌트가 이 동작에 의존하는 경우 고차 컴포넌트 패턴에 비해 버그가 발생할 위험이 높아질 수 있습니다.

### 고차 컴포넌트의 모범 사용 사례:

- 애플리케이션 전체에서 여러 컴포넌트에 동일하고 사용자 정의되지 않은 동작을 사용해야 하는 경우.
- 컴포넌트는 사용자 정의 로직을 추가하지 않고 독립적으로 작동할 수 있습니다.

### 훅의 모범 사용 사례:

- 동작이 사용되는 각각의 컴포넌트에서 사용자 정의되어야 합니다.
- 동작이 애플리케이션 전체에 사용되지 않고, 하나 또는 몇 개의 컴포넌트에서만 사용됩니다.
- 컴포넌트에 많은 속성을 추가합니다.

## 사례 연구

고차 컴포넌트 패턴에 의존하던 일부 라이브러리는 릴리스 이후 훅 지원을 추가했습니다.
그 좋은 예가 [Apollo Client](https://www.apollographql.com/docs/react/)입니다.
...
컴포넌트가 여러 resolvere에 접근해야 하는 경우, 이를 위해 여러 개의 graphql() 상위 컴포넌트를 작성해야 합니다.
여러 개의 고차 컴포넌트를 구성하면 데이터가 컴포넌트에 전달되는 방식을 이해하기 어려울 수 있습니다.
경우에 따라 고차 컴포넌트의 순서가 중요할 수 있으며, 이는 코드를 리팩토링할 때 쉽게 버그로 이어질 수 있습니다.

훅이 출시된 후 Apollo는 Apollo Client 라이브러리에 훅 지원을 추가했습니다. 이제 개발자는 graphql() 고차 컴포넌트를 사용하는 대신 라이브러리가 제공하는 훅을 통해 데이터에 직접 접근할 수 있습니다.

앞서 예제에서 보았던 graphql() 고차 컴포넌트를 사용한 예제와 정확히 동일한 데이터를 사용하는 예제를 살펴보겠습니다. 이번에는 Apollo Client가 제공한 useMutation 훅을 사용하여 데이터를 컴포넌트에 제공하겠습니다.
...
상용구가 줄어든 것 외에도 컴포넌트에서 여러 리졸버의 데이터를 사용하는 것이 훨씬 쉬워졌습니다. 여러 개의 상위 컴포넌트를 작성할 필요 없이 컴포넌트에 여러 개의 훅을 작성하기만 하면 됩니다. 이렇게 하면 데이터가 컴포넌트에 전달되는 방식을 훨씬 쉽게 파악할 수 있으며, 컴포넌트를 리팩토링하거나 더 작은 조각으로 나눌 때 개발자 경험을 개선할 수 있습니다.

## 장점

고차 컴포넌트 패턴을 사용하면 재사용하려는 로직을 모두 한 곳에 보관할 수 있습니다. 이렇게 하면 코드를 복제하다가 실수로 애플리케이션 전체에 버그를 퍼뜨려 매번 새로운 버그가 발생할 위험을 줄일 수 있습니다. 로직을 모두 한 곳에 보관하면 코드가 깔끔하게 유지되고 관심사를 쉽게 분리할 수 있습니다.
